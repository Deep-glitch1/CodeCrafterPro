...
class AIAssistant {
    ...
    async generateCodeFromPrompt() {
        ...
        try {
            ...
            if (mode === 'both' && app?.editors?.c && app?.editors?.custom) {
            ...
            } else if (mode === 'python' && app?.editors?.main) {
                app.editors.main.setValue(generated.pythonCode || generated.cCode || '');
                this.showNotification('Python code generated!', 'success');
                ...
                if (editor) {
                    editor.setValue(generated.cCode || generated.customCode || generated.pythonCode || '');
                    this.showNotification('Code generated successfully!', 'success');
                }
            }
            
            const output = document.getElementById('executionOutput');
            const preview = generated.cCode && generated.customCode
                ? `${generated.cCode}\n\n---\n\n${generated.customCode}`
                : (generated.pythonCode || generated.cCode || generated.customCode || '');
            if (output) {
                output.textContent = `✅ AI Generated Code:\n\n${preview}`;
                showOutputTab('executionOutput');
            } else {
                this.showNotification('Generated code, but output panel is unavailable.', 'warning');
            }
        } catch (error) {
...
likho note`;
                ...
                const basePython = `# Generated from: "${prompt}"
name = "Coder"
print("Hello from AI-generated Python code!")
print("Your request:", "${prompt}")`;

                if (mode === 'both') {
                    resolve({ cCode: baseC, customCode: baseCustom });
                } else if (mode === 'python') {
                    resolve({ pythonCode: basePython });
                } else if (mode === 'custom') {
...
if operator == "+":
    ...
    print("Unsupported operator")`
        ...
        if (!template) {
            this.showNotification('No matching template found.', 'warning');
            return;
        }
        ...
        if (editor) {
            if (mode === 'custom') {
                ...
                editor.setValue(template.python || template.c);
            } else {
